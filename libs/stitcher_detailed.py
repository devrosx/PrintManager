...
def main():
    ...
    if not timelapse:
        ...
        # base_name = os.path.basename(result_name)
        # print (result_name)
        # print (base_name)
        # name, ext = os.path.splitext(base_name)
        # stitched_name = f"{name}_stitched{ext}"
        first_image_path = img_names[0]
        base_name, ext = os.path.splitext(first_image_path)
        cv.imwrite(f"{base_name}_stitched{ext}", result)
        # print(f"Obrázek uložen jako: {stitched_name}")
        print (f"{base_name}_stitched{ext}")
